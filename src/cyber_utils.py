...
from config import WALLET_SEED, LCD_CLIENT, BASE_COIN_DENOM
...
def create_cls(link_candidates: list[list[str]], account_seed: str = WALLET_SEED, print_message: bool = False,
    ...
    _wallet = LCD_CLIENT.wallet(key=_key)
    ...
    _result = json.loads(LCD_CLIENT.tx.broadcast(_tx).to_json())
    if print_message:
        pprint(_result)
    return _result
...
def create_subgraph_links(link_candidates: list[list[str]], wallet: Wallet, subgraph_contract_address: str,
    ...
    return execute_contract(
        ...
        lcd_client=LCD_CLIENT, memo=memo, gas=gas, fee_amount=fee_amount, fee_denom=fee_denom)
